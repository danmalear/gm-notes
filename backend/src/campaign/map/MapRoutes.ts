...
export class MapRoutes {
	static readonly apiNamespace = 'maps';
	...
	async buildRegions(mapId: UUID) {
		const regions = await this.regionRepository.getByMapId(mapId);
	...
	async buildResponse(map: Map) {
		...
		const regions = await this.buildRegions(map.MapId);
	...
	init(app: Express) {
		app.get(
			...
			async (
				...
				const maps = req.query.campaignId
					? await this.mapRepository.getByCampaignId(req.query.campaignId)
					: await this.mapRepository.getAll();

				const data: MapResponse[] = [];

				try {
					for (const map of maps) {
						data.push(await this.buildResponse(map));
					}
				} catch (e) {
					res.status(500).send({
		...
		app.get(
			`/${MapRoutes.apiNamespace}/:id`,
			async (
				req,
				res: Response<MessageResponse | DataResponse<MapResponse>>,
			) => {
				console.log(
				...
				const map = await this.mapRepository.getById(req.params.id);
				if (!map) {
					res.status(404).send({
				...
				try {
					res.send({ data: await this.buildResponse(map) });
				} catch (e) {
					res.status(500).send({
						message: getMessage(e),
					});
		...
		app.post(
			...
			async (
				...
				map = await this.mapRepository.insert(map);
		...
		app.put(
			...
			async (
				...
				const updatedMap = await this.mapRepository.update(req.body.id, map);

				res.send({ data: await this.buildResponse(updatedMap) });
			},
		);
	}
}
